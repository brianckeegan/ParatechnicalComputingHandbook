...
**See also:** @sec-collaboration, @sec-pre-commit, @sec-git-github.
:::
...
## 4. `ruff`: the fast linter (and formatter)
...
## 5. Editor integration: format on save
...
## 6. Rule sets: what do E, F, I, B, UP mean?
...
Wire this into the same workflow that runs `pytest` (see @sec-testing) and every push gets automatic style enforcement.
...
-   Run `ruff check .` and `ruff format .` before every commit (or automate with @sec-pre-commit).
