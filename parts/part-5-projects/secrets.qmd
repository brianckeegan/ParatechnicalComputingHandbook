...
## 4. `python-dotenv`: loading a `.env` file into your program
...
## 5. `.gitignore`: the line that matters most in this chapter
...
## 6. Secrets in Jupyter notebooks
...
2.  **Clear outputs before committing.** `Cell → All Output → Clear` in Jupyter Lab, or use a git pre-commit hook (see @sec-pre-commit) to strip outputs automatically.
...
## 7. What if I already leaked a secret?
